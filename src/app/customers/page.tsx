          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mt-12 grid grid-cols-1 gap-6 lg:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 p-6 shadow-sm">
                <h3 className="mb-4 text-lg font-bold text-slate-900 dark:text-white">{t("customers.status_distribution")}</h3>
                <BarChart data={chartData} color="#dc2626" height={300} />
              </div>
              <div className="rounded-2xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 p-6 shadow-sm">
                <h3 className="mb-4 text-lg font-bold text-slate-900 dark:text-white">{t("customers.key_metrics")}</h3>
                <div className="space-y-4">
                  <div className="flex items-center justify-between rounded-lg bg-slate-50 dark:bg-slate-700 p-4">
                    <span className="text-sm font-semibold text-slate-600 dark:text-slate-400">{t("customers.active_rate")}</span>
                    <span className="text-xl font-bold text-slate-900 dark:text-white">
                      {stats.total > 0 ? ((stats.active / stats.total) * 100).toFixed(1) : 0}%
                    </span>
                  </div>
                  <div className="flex items-center justify-between rounded-lg bg-slate-50 dark:bg-slate-700 p-4">
                    <span className="text-sm font-semibold text-slate-600 dark:text-slate-400">{t("customers.inactive_rate")}</span>
                    <span className="text-xl font-bold text-slate-900 dark:text-white">
                      {stats.total > 0 ? ((stats.inactive / stats.total) * 100).toFixed(1) : 0}%
                    </span>
                  </div>
                  <div className="flex items-center justify-between rounded-lg bg-slate-50 dark:bg-slate-700 p-4">
                    <span className="text-sm font-semibold text-slate-600 dark:text-slate-400">{t("customers.blacklist_rate")}</span>
                    <span className="text-xl font-bold text-slate-900 dark:text-white">
